/* ============================================================================
   ...
   UPPDATERING (2026-09-14): Vercel ver2
   Här skedde en uppdatering: valfria anrop till /api/elpris och /api/scenario
   lades till (se DEL 8.6). Beräkningsformlerna i DEL 3 är oförändrade.
...
function lasAllaIndataFalt() {
  const falt = {};
  document.querySelectorAll('.indata[id]').forEach((el) => {
    falt[el.id] = el.value;
  });
  return falt;
}

function fyllIndataFalt(falt) {
  if (!falt || typeof falt !== 'object') return;
  Object.keys(falt).forEach((id) => {
    const el = document.getElementById(id);
    if (el && el.classList.contains('indata')) {
      el.value = falt[id];
    }
  });
}
...
/**
 * Bekräftar jämförelsetabellen som om användaren lämnat ett fält,
 * så att "Hämta spotpris" syns som en förändring.
 */
function bekraftaAndring() {
  tidigareResultat = senasteResultat;
  senasteResultat = omrakna();
  ritaJamforelse();
}
...
async function hamtaAktuelltElpris() {
  const knapp = document.getElementById('hamta-elpris');
  const omrade = text('elomrade') || 'SE4';
  visaVerktygStatus('Hämtar spotpris för ' + omrade + ' …');
  if (knapp) knapp.disabled = true;
  try {
    const res = await fetch('./api/elpris?omrade=' + encodeURIComponent(omrade));
    const data = await res.json().catch(() => null);
    if (!res.ok || !data || data.ok === false) {
      const fel = (data && data.fel) || 'Kunde inte hämta elpris (' + res.status + ').';
      visaVerktygStatus(fel + ' Skriv in priset manuellt.', 'fel');
      return;
    }
    if (data.prisKrKwh == null) {
      visaVerktygStatus(
        (data.fxFel || 'Priset kom i EUR/MWh men kunde inte räknas om till kr.') +
          ' Skriv in kr/kWh manuellt.',
        'fel'
      );
      return;
    }

    const spot = document.getElementById('spotpris');
    if (spot) spot.value = data.prisKrKwh;
    const somIntakt = document.getElementById('elpris-som-intakt');
    if (somIntakt && somIntakt.checked) {
      const elpris = document.getElementById('elpris');
      if (elpris) elpris.value = data.prisKrKwh;
    }

    bekraftaAndring();

    const eur = Number(data.prisEurMwh).toLocaleString('sv-SE', { maximumFractionDigits: 2 });
    const kr = Number(data.prisKrKwh).toLocaleString('sv-SE', { maximumFractionDigits: 4 });
    visaVerktygStatus(
      omrade + ' ' + data.leveransdag + ': ' + kr + ' kr/kWh (dygnssnitt, ' +
        eur + ' EUR/MWh via ENTSO-E, omräknat med Riksbanken). ' +
        (data.varning || ''),
      'ok'
    );
  } catch (err) {
    visaVerktygStatus(
      'Nätfel mot /api/elpris. Lokalt utan Vercel, eller tillfällig störning. Skriv in priset manuellt.',
      'fel'
    );
  } finally {
...
async function sparaOchKopieraLank() {
  ...
  try {
    const res = await fetch('./api/scenario', {
      ...
      body: JSON.stringify({ falt: lasAllaIndataFalt() })
    });
    const data = await res.json().catch(() => null);
    if (!res.ok || !data || data.ok === false) {
...
async function laddaScenarioFranUrl() {
  const params = new URLSearchParams(window.location.search);
  const id = params.get('s');
  const token = params.get('t');
  if (!id && !token) return;

  const q = id ? ('id=' + encodeURIComponent(id)) : ('token=' + encodeURIComponent(token));
  try {
    const res = await fetch('./api/scenario?' + q);
    const data = await res.json().catch(() => null);
    if (!res.ok || !data || !data.falt) {
      visaVerktygStatus((data && data.fel) || 'Kunde inte läsa delad kalkyl.', 'fel');
      return;
    }
    fyllIndataFalt(data.falt);
    visaVerktygStatus('Delat scenario inläst.', 'ok');
  } catch (err) {
    visaVerktygStatus('Kunde inte läsa delad kalkyl (API nås inte).', 'fel');
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  kopplaVercelVerktyg(); // UPPDATERING (2026-09-14): elpris + dela-länk

  // Första beräkningen vid sidladdning. Om URL:en bär ett scenario
  // fylls fälten först, så att omräkningen speglar det delade läget.
  laddaScenarioFranUrl().then(() => {
